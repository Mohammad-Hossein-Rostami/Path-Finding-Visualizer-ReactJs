...
import {
  greedy,
  greedyGetNodesInShortestPathOrder,
...
export const PathfindingVisualizer = () => {
  ...
  function visualize() {
    if (!haveFinish || !haveStart) {
      window.alert("Please Determine the Start Point...");
    }
    ...
    document.getElementById(`node-${startPos.r}-${startPos.c}`).className =
      "node node-start";
    document.getElementById(`node-${finishPos.r}-${finishPos.c}`).className =
      "node node-finish";

    if (selectedAlgo === "Astar") {
      const visitedNodesInOrder = Astar(grid, startNode, finishNode);
      const nodesInShortestPathOrder =
        AstarGetNodesInShortestPathOrder(finishNode);
    ...
    if (selectedAlgo === "BFS") {
      ...
      const nodesInShortestPathOrder =
        bfsGetNodesInShortestPathOrder(finishNode);
      setPath(nodesInShortestPathOrder.length);
      setSearched(visitedNodesInOrder.length);
      animate(visitedNodesInOrder, nodesInShortestPathOrder);
    }
    ...
    if (selectedAlgo === "DFS") {
      ...
      const nodesInShortestPathOrder =
        dfsGetNodesInShortestPathOrder(finishNode);
      setPath(nodesInShortestPathOrder.length);
      setSearched(visitedNodesInOrder.length);
      animate(visitedNodesInOrder, nodesInShortestPathOrder);
    }
    ...
    if (selectedAlgo === "Greedy") {
      ...
      const nodesInShortestPathOrder =
        greedyGetNodesInShortestPathOrder(finishNode);
    ...
    if (selectedAlgo === "all") {
      ...
      const greedyNodesInShortestPathOrder =
        greedyGetNodesInShortestPathOrder(finishNode);
      ...
      const dfsNodesInShortestPathOrder =
        dfsGetNodesInShortestPathOrder(finishNode);
      ...
      const bfsNodesInShortestPathOrder =
        bfsGetNodesInShortestPathOrder(finishNode);
      ...
      const AstarNodesInShortestPathOrder =
        AstarGetNodesInShortestPathOrder(finishNode);
    ...
    <>
      <nav>
          ...
          <ul>
            <li>
              <select onChange={(e) => setSelectedAlgo(e.target.value)}>
                <option value="Astar">Choose An Algorithm ( Click ) </option>
                ...
                <option value="all">Run All Algorithms Together</option>
              </select>
            </li>
          </ul>
        </div>
        <div className="center">
          <ul>
            <li>
              <button className="start-algorithm" onClick={() => visualize()}>
                Run The Algorithm <b style={{ color: "red" }}>{selectedAlgo}</b>
              </button>
            </li>
            <li>
        ...
        <div className="left">
          <ul>
            <li> Algorithm Path Visualizer </li>
          </ul>
        </div>
      </nav>
      ...
      <div className="guid-nav">
        <ul>
          <li id="guid-1">
            <div className="wall"></div> obstacle
          </li>
          <li id="guid-2">
            <div className="path"></div> Founded Path
          </li>
          <li id="guid-3">
            <div className="searched"></div> Searched Nodes
          </li>
          <li id="guid-4">
            <div className="goal"></div> Goal Point
          </li>
          <li id="guid-5">
            {" "}
            <div className="start"></div> Start Point
          </li>
        </ul>
      </div>
      ...
      <div className="count">
        <h4>Walls: {walls}</h4>
        <h4>Searched Nodes: {searched}</h4>
        <h4>Shortest Path: {path}</h4>
      </div>
